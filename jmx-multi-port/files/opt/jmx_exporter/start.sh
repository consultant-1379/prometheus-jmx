...
echo "---
hostPort: XXX_HOST_XXX:XXX_PORT_XXX
username:
password:" > /opt/jmx_exporter/config/config.yml.template
...
EXPORTER_JAR="/opt/jmx_exporter/exporter/jmx_prometheus_httpserver-$VERSION.jar"
...
# Debug block...
  echo "Environment variables set/received..."
...
if [ "$PREPARE_CONFIG" = "true" ]; then
  ...
  # config inline...
  IFS=', ' read -r -a DEST_PORTS <<< "$DEST_PORT";

  iterations=0
  for PORT in "${DEST_PORTS[@]}";
  do
    CONFIG_FILE="$CONFIG_DIR/config$((++iterations)).yml"
    ...
    cat "$CONFIG_TEMPLATE" | sed -e "s/XXX_HOST_XXX/$DEST_HOST/" -e "s/XXX_PORT_XXX/$PORT/" > $CONFIG_FILE
...
java $JVM_LOCAL_OPTS -jar ${EXPORTER_JAR} $SERVICE_PORT ${CONFIG_FILES[@]/#/}